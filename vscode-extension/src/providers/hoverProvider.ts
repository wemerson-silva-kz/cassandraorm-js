...
export class SchemaHoverProvider implements vscode.HoverProvider {
    ...
    provideHover(
        ...
        if (typeDoc) return typeDoc;

        // Method documentation
        const methodDoc = this.getMethodDocumentation(word, line);
        if (methodDoc) return methodDoc;

        // Validation documentation
        const validationDoc = this.getValidationDocumentation(word, line);
        if (validationDoc) return validationDoc;

        return null;
    }

    private getTypeDocumentation(word: string): vscode.Hover | null {
    ...
    private getMethodDocumentation(word: string, line: string): vscode.Hover | null {
        const methods: Record<string, { description: string, example: string }> = {
            'find': {
    ...
    private getValidationDocumentation(word: string, line: string): vscode.Hover | null {
